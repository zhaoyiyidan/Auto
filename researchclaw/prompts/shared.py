...
_DEFAULT_BLOCKS: dict[str, str] = {
    ...
    "stage10_validation_boundary": (
        ...
        "0. Stage 9 plan sections that say to run the experiment describe what "
        "ResearchClaw Stage 12 will do later. In Stage 10, convert that into a "
        "manifest launch command; do not execute it.\n"
...
_DEFAULT_SUB_PROMPTS: dict[str, dict[str, Any]] = {
    "workspace_codegen": {
        "system": (
            ...
            "repository. Implement the Stage 9 experiment plan. Do not redesign the "
            "experiment and do not emit code blocks for ResearchClaw to parse."
        ),
        "user": (
            "TOPIC:\n{topic}\n\n"
            "STAGE 9 PLAN (plan.md):\n{plan_md}\n\n"
            "EXPECTED OUTPUTS (expected_outputs.json):\n{expected_outputs_json}\n\n"
            "Completion contract (MUST):\n"
            "1. MUST inspect the existing workspace before editing. If a working "
            "experiment entry point already satisfies the plan, reuse it.\n"
            "2. MUST make only the minimal repository changes needed for this task, "
            "using the existing structure; do not rewrite working experiment code.\n"
            ...
            "launch.env, launch.resources, and result_paths in the manifest.\n"
            "6. MUST include every path listed in expected_outputs.json in "
            "manifest.result_paths. Extra result paths are allowed.\n"
            "7. MUST commit the experiment implementation after all code/data/script "
            "changes are ready.\n"
            "8. MUST set manifest.code_commit to the git commit that contains the "
            "experiment implementation to execute. If you commit {manifest_filename} "
            "separately after the implementation commit, code_commit should point to "
            "the implementation commit, not necessarily final HEAD.\n"
            "9. MUST commit {manifest_filename} and finish by verifying "
    ...
    "workspace_repair": {
        ...
        "user": (
            ...
            "Completion contract (MUST):\n"
            "1. MUST inspect the existing workspace before editing. If a working "
            "experiment entry point already satisfies the repair request, reuse it.\n"
            "2. MUST make only the minimal repository changes needed for this repair, "
            "using the existing structure; do not rewrite working experiment code.\n"
            "3. MUST prepare a launch command or script for the experiment run.\n"
            "4. MUST write {manifest_filename} in the workspace root or .researchclaw/.\n"
            "5. MUST include schema_version, code_commit, launch.command, launch.cwd, "
            ...
            "6. MUST include every path listed in expected_outputs.json in "
            "manifest.result_paths. Extra result paths are allowed.\n"
            "7. MUST commit the repaired experiment implementation after all "
            "code/data/script changes are ready.\n"
            "8. MUST set manifest.code_commit to the git commit that contains the "
            "experiment implementation to execute. If you commit {manifest_filename} "
            "separately after the implementation commit, code_commit should point to "
            "the implementation commit, not necessarily final HEAD.\n"
    ...
    "experiment_repair_instructions": {
        ...
        "user": (
            ...
            "- Commit the experiment implementation after all code changes are ready.\n"
            "- Set run_manifest.json code_commit to the implementation commit that the harness should execute.\n"
            "- Commit run_manifest.json and leave the workspace clean.\n"
